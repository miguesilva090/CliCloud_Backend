...
namespace CliCloud.Application.Services.Prescricao.MedicacaoFavoritaService.DTOs
{
    public class CreateMedicacaoFavoritaRequest : IDto 
    {
    ...
    public class CreateMedicacaoFavoritaRequestValidator : AbstractValidator<CreateMedicacaoFavoritaRequest>
    {
        public CreateMedicacaoFavoritaRequestValidator()
        {
            ...
            _ = RuleFor(x => x.TipoLinha).Must(t => t is 1 or 3)
                .WithMessage("Não é permitido adicionar aos favoritos para este tipo Prescrição");
        }
    }
}