...
public interface ILoteDirectService : ITransientService
{
    ...
    Task<Response<PassarParaAtivoResultDTO>> PassarParaAtivoAsync(PassarParaAtivoRequest request);
    Task<Response<ObterNovoLoteResultDTO>> ObterNovoLoteAsync(ObterNovoLoteRequest request);
}