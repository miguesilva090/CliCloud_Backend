...
public sealed class CredenciaisSnsFisioterapiaSearchSpec : Specification<LoteFisioterapia>
{
    public CredenciaisSnsFisioterapiaSearchSpec(List<TableFilter> filters, string? dynamicOrder = "")
    {
        ...
        if (string.IsNullOrWhiteSpace(dynamicOrder))
            Query.OrderBy(x => x.CodigoOrganismo).ThenBy(x => x.NumeroLote);
        else
