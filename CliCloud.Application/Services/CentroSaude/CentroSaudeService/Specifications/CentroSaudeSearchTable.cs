...
{
    public class CentroSaudeSearchTable : Specification<CentroSaudeEntity>
    {
        public CentroSaudeSearchTable(List<TableFilter> filters, string? dynamicOrder = "")
        {
              ...
              foreach(TableFilter filter in filters)
              {
                switch((filter.Id ?? "").ToLower(System.Globalization.CultureInfo.InvariantCulture))
                {
                  ...
                  case "codigolocalcs":
                    if(!string.IsNullOrWhiteSpace(filter.Value))
                    {
                      _ = Query.Where(x => x.CodigoLocalCS != null && x.CodigoLocalCS.Contains(filter.Value));
                    }
                    break;
                  case "rua.nome":
                    if(!string.IsNullOrWhiteSpace(filter.Value))
                  ...
                  case "entidadetipoid":
                    ...
                    {
                      if(entidadeTipoId == 7) // CentroSaude = 7
                      {
                        _ = Query.Where(x => x.TipoEntidade == EntidadeTipo.CentroSaude);
                      }
                    }
                    break;
                  default:
