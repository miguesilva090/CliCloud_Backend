...
{
    public class MedicoSearchTable : Specification<Medico>
    {
        public MedicoSearchTable(List<TableFilter> filters, string? dynamicOrder = "")
        {
              ...
              foreach(TableFilter filter in filters)
              {
                switch((filter.Id ?? "").ToLower(System.Globalization.CultureInfo.InvariantCulture))
                {
                  ...
                  case "numerocartaoidentificacao":
                    ...
                    break;
                  case "director":
                    if(!string.IsNullOrWhiteSpace(filter.Value) && bool.TryParse(filter.Value, out bool director))
                    {
                      _ = Query.Where(x => x.Director == director);
                    }
                    break;
